...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get(
    'DATABASE_URL',
    'postgresql://user:password@localhost:5432/hsc_academy'
)
...
def update_student(sid):
    ...
    allowed = ['name', 'roll', 'reg', 'cls', 'group', 'section', 'father', 'mother',
               'dob', 'phone', 'religion', 'year', 'session', 'photo']
...
def public_result_search():
    ...
    peer_scores = []
    for peer in peers:
        t, g, _ = _compute_student_result(peer.id, all_marks, peer.group)
...
def _compute_student_result(sid, marks_data, group):
...
def analyze_promotion():
    ...
    for stu in students:
        total, gpa, passed = _compute_student_result(stu.id, marks, stu.group)
        data = stu.to_dict()
        data.update({
            'total_marks': total,
            'gpa':         gpa,
...
def execute_promotion():
    ...
    for stu in to_promote:
        total, gpa, _ = _compute_student_result(stu.id, marks, stu.group)
...
def generate_certificate(sid):
    ...
    marks           = _get_marks_dict()
    total, gpa, passed = _compute_student_result(sid, marks, getattr(stu, 'group', 'Science'))
